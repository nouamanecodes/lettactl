...
export async function describeAgent(
  ...
  try {
    ...
    const displayData: AgentDetailsData = {
      ...
      model: agentDetails.metadata?.['lettactl.model'] || agentDetails.llm_config?.handle || agentDetails.llm_config?.model,
      contextWindow: agentDetails.llm_config?.context_window,
      embedding: agentDetails.metadata?.['lettactl.embedding'] || agentDetails.embedding_config?.embedding_model,
