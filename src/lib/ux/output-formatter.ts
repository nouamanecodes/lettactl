...
export class OutputFormatter {
   ...
   */
  static showAgentUpdateDiff(operations: AgentUpdateOperations, builtinTools?: Set<string>, force: boolean = false, prune: boolean = false): void {
    ...
    if (operations.tools) {
      ...
      if (toAdd.length > 0 || toRemove.length > 0 || toUpdate.length > 0) {
        ...
        toRemove.forEach(tool => log(`    - Removed tool: ${tool.name}${getBuiltinTag(tool.name)}${(force || prune) ? '' : ' (requires --prune)'}`));
    ...
    if (operations.blocks) {
      ...
      if (toAdd.length > 0 || toRemove.length > 0 || toUpdate.length > 0 || toUpdateValue.length > 0) {
        ...
        toRemove.forEach(block => log(`    - Removed block: ${block.name}${(force || prune) ? '' : ' (requires --prune)'}`));
